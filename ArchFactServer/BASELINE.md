...
默认 OCR 为 PaddleOCR 3.x `PP-OCRv6_small`（权重在 `models/paddleocr`，不进 git）；
worker 就绪后才开始计页面超时，页缓存按模型版本命中。启动时恢复中断的抽取任务
并跳过已完成页面；硬件自适应会按本机重算 OCR/发现并发。领域辅助、视图映射和
Mongo 仓储已按边界拆分，抽取路由前缀保持 `/extraction-jobs`。
...
`models/archaeology-yolo/v1/best.pt`。PaddleOCR 3.x 权重通过
`scripts/download_ppocrv6_small.py` 下载到 `models/paddleocr`。这些机器相关内容
被明确排除，不会随基线提交。
