        ...
        "fields": [
            {
                ...
                "type": "string",
                "instruction": (
                    "只提取原文明确出现、带冒号的完整器物编号（如 H125:1、M3:18、T3:3）；"
                    "不得把仅有遗迹号/墓葬号的 H125、M3、T3，当作器物编号，"
                    "不得把图号或图中序号当作器物编号。"
                ),
        ...
        "fields": [
            {
                ...
                "instruction": (
                    ...
                    "只有同一器物条目中同时出现冒号后的序号时才填写本字段；"
                    "单独出现的遗迹/墓葬标题（如 M14）不得生成器物记录。"
                    "类型学标题（如 B型、I式）只作为后续条目的上下文；"
                    "图中 BI(M5:1) 的遗迹号应提取为 M5，不得提取 BI。"
                ),
                "evidence_kind": "number",
            },
            {
                ...
                "instruction": (
                    ...
                    "只保留序号本身，不得混入图号或图中序号。"
                    "若未在同一器物条目中看到完整的‘遗迹号:序号’，本字段留空且不得生成器物记录。"
                    "图中 BI(M5:1) 的序号应提取为 1；BI 仅表示 B型 I式，不是器物编号。"
                ),
                "evidence_kind": "number",
            },
            {
